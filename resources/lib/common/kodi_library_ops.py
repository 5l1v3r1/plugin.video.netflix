...
def scan_library(path=""):
    """Start a library scanning in a specified folder to find new items"""
...
def clean_library():
    """Start a library scanning to remove non-existing items"""
    method = 'VideoLibrary.Clean'
    return json_rpc(method)
    ...
    try:
        ...
        if videoid.mediatype in [VideoId.SHOW, VideoId.SEASON]:
            ...
            if videoid.mediatype == VideoId.SEASON:
                # From the single file result, get the Kodi season
                # so add a season filter to get all episodes of the specified season
